...
-- Charmeleon page 1 for the pokedex
...
local function handleButtonEvent( event )
    if ( "ended" == event.phase ) then
		composer.gotoScene("charmeleon3")
...
local function playcry (event)
	if  ( "ended" == event.phase) then 
		local cry = audio.loadSound("5.mp3")
...
local previousPage = widget.newButton(
	...
	local image = display.newImageRect( "005.png", 120,120)		
	image.anchorX = 0
	image.anchorY = 0
	sceneGroup:insert(image)
	--Display pokemon name and pokedex info
	local name = display.newText( "Charmeleon", display.contentWidth*(5/8) , 60, native.systemFont, 20 )
	...
	local redblueEntry = display.newText("When it swings its burning tail, it elevates the temperature to unbearably high levels.", display.contentWidth/2, 170, 310,0,right,native.systemFont, 15)
	...
	local yellowEntry = display.newText("Tough fights could excite this Pokémon. When excited, it may blow out bluish-white flames.", display.contentWidth/2, 350,310,0,right, native.systemFont, 15)
