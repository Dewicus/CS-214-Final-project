...
-- Charmeleon page 3 for the pokedex
...
local function handleButtonEvent( event )
    if ( "ended" == event.phase ) then
		composer.gotoScene("charmeleon4")
...
local function handlepreviousPageEvent (event)
	if  ( "ended" == event.phase) then 
		composer.gotoScene("charmeleon2")
	end
end
...
local previousPage = widget.newButton(
	...
	local GoldEntry = display.newText("It is very hot-headed by nature, so it constantly seeks opponents. It calms down only when it wins.", display.contentWidth/2, 175,310,0,right, native.systemFont, 15)
	...
	local SilverEntry = display.newText("It has a barbaric nature. In battle, it whips its fiery tail around and slashes away with sharp claws.", display.contentWidth/2, 330,310,0,right, native.systemFont, 15)
	...
	local CrystalEntry = display.newText("If it becomes agitated during battle, it spouts intense flames, incinerating its surroundings.", display.contentWidth/2, 405,310,0,right, native.systemFont, 15)
