...
-- Butterfree page 3 for the pokedex
...
local function handleButtonEvent( event )
    if ( "ended" == event.phase ) then
		composer.gotoScene("butterfree4")
...
local function handlepreviousPageEvent (event)
	if  ( "ended" == event.phase) then 
		composer.gotoScene("butterfree2")
	end
end
...
local previousPage = widget.newButton(
	...
	local GoldEntry = display.newText("It collects honey every day. It rubs honey onto the hairs on its legs to carry it back to its nest.", display.contentWidth/2, 175,310,0,right, native.systemFont, 15)
	...
	local SilverEntry = display.newText("Water-repellent powder on its wings enables it to collect honey, even in the heaviest of rains.", display.contentWidth/2, 330,310,0,right, native.systemFont, 15)
	...
	local CrystalEntry = display.newText("It flits from flower to flower, collecting honey. It can even identify distant flowers in bloom.", display.contentWidth/2, 410,310,0,right, native.systemFont, 15)
