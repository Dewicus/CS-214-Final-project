...
-- Beedrill page 1 for the pokedex
...
local function playcry (event)
	if  ( "ended" == event.phase) then 
		local cry = audio.loadSound("15.mp3")
...
local previousPage = widget.newButton(
	...
	local image = display.newImageRect( "015.png", 120,120)		
	image.anchorX = 0
	image.anchorY = 0
	sceneGroup:insert(image)
	--Display pokemon name and pokedex info
	local name = display.newText( "Beedrill", display.contentWidth*(5/8) , 60, native.systemFont, 20 )
	name:setFillColor( 0, 0, 0 )
	sceneGroup:insert(name)
	local poketype = display.newText( "The Poison Bee Pokemon", display.contentWidth*(5/8), 80, native.systemFont, 15)
	...
	--games red and blue
	local redblue = display.newText("Red and Blue Pokedex entry:", display.contentWidth/2, 130, native.systemFont, 17)
	redblue:setFillColor (0,0,0)
	sceneGroup: insert(redblue)
	local redblueEntry = display.newText("Flies at high speed and attacks using its large venomous stingers on its forelegs and tail.", display.contentWidth/2, 170, 310,0,right,native.systemFont, 15)
	redblueEntry:setFillColor (0,0,0)
	sceneGroup: insert(redblueEntry)
	
	--yellow
	local yellow = display.newText("Yellow Pokedex entry:", display.contentWidth/2, 310, native.systemFont, 17)
	yellow:setFillColor (0,0,0)
	sceneGroup: insert(yellow)
	local yellowEntry = display.newText("It has 3 poisonous stingers on its forelegs and its tail. They are used to jab its enemy repeatedly.", display.contentWidth/2, 350,310,0,right, native.systemFont, 15)
	yellowEntry:setFillColor (0,0,0)
	sceneGroup: insert(yellowEntry)
end
