...
local function playcry (event)
	if  ( "ended" == event.phase) then 
		local cry = audio.loadSound("13.mp3")
...
local previousPage = widget.newButton(
	...
	local image = display.newImageRect( "013.png", 120,120)		
	image.anchorX = 0
	image.anchorY = 0
	sceneGroup:insert(image)
	--Display pokemon name and pokedex info
	local name = display.newText( "Weedle", display.contentWidth*(5/8) , 60, native.systemFont, 20 )
	name:setFillColor( 0, 0, 0 )
	sceneGroup:insert(name)
	local poketype = display.newText( "The Hairy Bug Pokemon", display.contentWidth*(5/8), 80, native.systemFont, 15)
	...
	local redblueEntry = display.newText("Often found in forests, eating leaves. It has a sharp venomous stinger on its head.", display.contentWidth/2, 170, 310,0,right,native.systemFont, 15)
	...
	local yellowEntry = display.newText("Beware of the sharp stinger on its head. It hides in grass and bushes where it eats leaves.", display.contentWidth/2, 350,310,0,right, native.systemFont, 15)
