...
-- Bulbasaur page 5 for the pokedex
...
local function handlepreviousPageEvent (event)
	if  ( "ended" == event.phase) then 
		composer.gotoScene("bulbasaur4")
	end
end
--play cry for invisible button
local function playcry (event)
	if  ( "ended" == event.phase) then 
		local cry = audio.loadSound("1.mp3")
...
local previousPage = widget.newButton(
	...
	local image = display.newImageRect( "001.png", 120,120)		
	image.anchorX = 0
	image.anchorY = 0
	sceneGroup:insert(image)
	--Display pokemon name and pokedex info
	local name = display.newText( "Bulbasaur", display.contentWidth*(5/8) , 60, native.systemFont, 20 )
	...
	local blEntry = display.newText("For some time after its birth, it grows by gaining nourishment from the seed on its back.", display.contentWidth/2, 175,310,0,right, native.systemFont, 15)
	...
	sceneGroup: insert(x)
	local xEntry = display.newText("A strange seed was planted on its back at birth. The plant sprouts and grows with this Pokémon.", display.contentWidth/2, 330,310,0,right, native.systemFont, 15)
	...
	sceneGroup: insert(y)
	local yEntry = display.newText("For some time after its birth, it grows by gaining nourishment from the seed on its back.", display.contentWidth/2, 395,310,0,right, native.systemFont, 15)
