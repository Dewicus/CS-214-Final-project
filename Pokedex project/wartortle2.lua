...
-- Wartortle page 1 for the pokedex
...
local function playcry (event)
	if  ( "ended" == event.phase) then 
		local cry = audio.loadSound("8.mp3")
...
local previousPage = widget.newButton(
	...
	local image = display.newImageRect( "008.png", 120,120)		
	image.anchorX = 0
	image.anchorY = 0
	sceneGroup:insert(image)
	--Display pokemon name and pokedex info
	local name = display.newText( "Wartortle", display.contentWidth*(5/8) , 60, native.systemFont, 20 )
	name:setFillColor( 0, 0, 0 )
	sceneGroup:insert(name)
	local poketype = display.newText( "The Turtle Pokemon", display.contentWidth*(5/8), 80, native.systemFont, 15)
	...
	local redblueEntry = display.newText("Often hides in water to stalk unwary prey. For swimming fast, it moves its ears to maintain balance.", display.contentWidth/2, 170, 310,0,right,native.systemFont, 15)
	redblueEntry:setFillColor (0,0,0)
	sceneGroup: insert(redblueEntry)
	
	--yellow
	local yellow = display.newText("Yellow Pokedex entry:", display.contentWidth/2, 320, native.systemFont, 17)
	yellow:setFillColor (0,0,0)
	sceneGroup: insert(yellow)
	local yellowEntry = display.newText("When tapped, this Pokémon will pull in its head, but its tail will still stick out a little bit.", display.contentWidth/2, 350,310,0,right, native.systemFont, 15)
