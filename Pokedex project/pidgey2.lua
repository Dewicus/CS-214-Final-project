...
local function playcry (event)
	if  ( "ended" == event.phase) then 
		local cry = audio.loadSound("16.mp3")
...
local previousPage = widget.newButton(
	...
	local image = display.newImageRect( "016.png", 120,120)		
	image.anchorX = 0
	image.anchorY = 0
	sceneGroup:insert(image)
	--Display pokemon name and pokedex info
	local name = display.newText( "Pidgey", display.contentWidth*(5/8) , 60, native.systemFont, 20 )
	name:setFillColor( 0, 0, 0 )
	sceneGroup:insert(name)
	local poketype = display.newText( "The Tiny Bird Pokemon", display.contentWidth*(5/8), 80, native.systemFont, 15)
	...
	local redblueEntry = display.newText("A common sight in forests and woods. It flaps its wings at ground level to kick up blinding sand.", display.contentWidth/2, 170, 310,0,right,native.systemFont, 15)
	...
	local yellowEntry = display.newText("Very docile. If attacked, it will often kick up sand to protect itself rather than fight back.", display.contentWidth/2, 340,310,0,right, native.systemFont, 15)
