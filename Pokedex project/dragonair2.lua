...
-- Dragonair page 2 for the pokedex
...
local function playcry (event)
	if  ( "ended" == event.phase) then 
		local cry = audio.loadSound("148.mp3")
...
local previousPage = widget.newButton(
	...
	local image = display.newImageRect( "148.png", 120,120)		
	image.anchorX = 0
	image.anchorY = 0
	sceneGroup:insert(image)
	--Display pokemon name and pokedex info
	local name = display.newText( "Dragonair", display.contentWidth*(5/8) , 60, native.systemFont, 20 )
	...
	local redblueEntry = display.newText("A mystical Pokémon that exudes a gentle aura. Has the ability to change climate conditions.", display.contentWidth/2, 170, 310,0,right,native.systemFont, 15)
	...
	local yellowEntry = display.newText("According to a witness, its body was surrounded by a strange aura that gave it a mystical look.", display.contentWidth/2, 350,310,0,right, native.systemFont, 15)
