...
-- Blastoise page 5 for the pokedex
...
local previousPage = widget.newButton(
	...
	local blEntry = display.newText("The jets of water it spouts from the rocket cannons on its shell can punch through thick steel.", display.contentWidth/2, 170,310,0,right, native.systemFont, 15)
	...
	sceneGroup: insert(x)
	local xEntry = display.newText("It crushes its foe under its heavy body to cause fainting. In a pinch, it will withdraw inside its shell.", display.contentWidth/2, 330,310,0,right, native.systemFont, 15)
	...
	sceneGroup: insert(y)
	local yEntry = display.newText("The pressurized water jets on this brutal Pokémon's shell are used for high-speed tackles.", display.contentWidth/2, 400,310,0,right, native.systemFont, 15)
