...
-- Charmander page 5 for the pokedex
...
local function handleButtonEvent( event )
    if ( "ended" == event.phase ) then
		composer.gotoScene("charmander6")
...
local previousPage = widget.newButton(
	...
	local blEntry = display.newText("The fire on the tip of its tail is a measure of its life. If healthy, its tail burns intensely.", display.contentWidth/2, 175,310,0,right, native.systemFont, 15)
	...
	sceneGroup: insert(x)
	local xEntry = display.newText("The flame on its tail indicates Charmander's life force. If it is healthy, the flame burns brightly.", display.contentWidth/2, 330,310,0,right, native.systemFont, 15)
	...
	sceneGroup: insert(y)
	local yEntry = display.newText("From the time it is born, a flame burns at the tip of its tail. Its life would end if the flame were to go out.", display.contentWidth/2, 405,310,0,right, native.systemFont, 15)
