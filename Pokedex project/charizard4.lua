...
-- Charizard page 4 for the pokedex
...
local previousPage = widget.newButton(
	...
	local rse = display.newText("Ruby, Sapphire and Emerald Pokedex	\n                          entries:", display.contentWidth/2, 133,310,0, native.systemFont, 17, "center") 
	rse:setFillColor (0,0,0)
	sceneGroup: insert(rse)
	local rseEntry = display.newText("Charizard flies around the sky in search of powerful opponents. It breathes fire of such \n\n\n\n\n\ngreat heat that it melts anything. However, it never turns its fiery breath on any opponent weaker than itself.", display.contentWidth/2, 247,312,0,right, native.systemFont, 15)
	rseEntry:setFillColor (0,0,0)
	sceneGroup: insert(rseEntry)
	
	--dimond pearl and platinum
	local dpp = display.newText("   Diamond, Pearl and Platinum Pokedex \n                          entries:", display.contentWidth/2, 365,310,0,center, native.systemFont, 17)
	dpp:setFillColor (0,0,0)
	sceneGroup: insert(dpp)
	local dppEntry = display.newText("It is said that Charizard's fire burns hotter if it has experienced harsh battles.", display.contentWidth/2, 400,310,0,right, native.systemFont, 15)
