...
-- Dragonite page 2 for the pokedex
...
local function playcry (event)
	if  ( "ended" == event.phase) then 
		local cry = audio.loadSound("149.mp3")
...
local previousPage = widget.newButton(
	...
	local image = display.newImageRect( "149.png", 120,120)		
	image.anchorX = 0
	image.anchorY = 0
	sceneGroup:insert(image)
	--Display pokemon name and pokedex info
	local name = display.newText( "Dragonite", display.contentWidth*(5/8) , 60, native.systemFont, 20 )
	...
	local redblueEntry = display.newText("An extremely rarely seen marine Pokémon. Its intelligence is said to match that of humans.", display.contentWidth/2, 170, 310,0,right,native.systemFont, 15)
	...
	local yellowEntry = display.newText("It is said that this Pokémon lives somewhere in the sea and that it flies. However, it is only a rumor.", display.contentWidth/2, 350,310,0,right, native.systemFont, 15)
