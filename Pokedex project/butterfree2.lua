...
-- Butterfree page 1 for the pokedex
...
local function handleButtonEvent( event )
    if ( "ended" == event.phase ) then
		composer.gotoScene("butterfree3")
...
local function playcry (event)
	if  ( "ended" == event.phase) then 
		local cry = audio.loadSound("12.mp3")
...
local previousPage = widget.newButton(
	...
	local image = display.newImageRect( "012.png", 120,120)		
	image.anchorX = 0
	image.anchorY = 0
	sceneGroup:insert(image)
	--Display pokemon name and pokedex info
	local name = display.newText( "Butterfree", display.contentWidth*(5/8) , 60, native.systemFont, 20 )
	name:setFillColor( 0, 0, 0 )
	sceneGroup:insert(name)
	local poketype = display.newText( "The Butterfly Pokemon", display.contentWidth*(5/8), 80, native.systemFont, 15)
	...
	local redblueEntry = display.newText("In battle, it flaps its wings at high speed to release highly toxic dust into the air.", display.contentWidth/2, 170, 310,0,right,native.systemFont, 15)
	...
	local yellowEntry = display.newText("Its wings, covered with poisonous powders, repel water. This allows it to fly in the rain.", display.contentWidth/2, 350,310,0,right, native.systemFont, 15)
