...
-- Charmander page 1 for the pokedex
...
local function handleButtonEvent( event )
    if ( "ended" == event.phase ) then
		composer.gotoScene("charmander3")
...
local function playcry (event)
	if  ( "ended" == event.phase) then 
		local cry = audio.loadSound("4.mp3")
...
local previousPage = widget.newButton(
	...
	local image = display.newImageRect( "004.png", 120,120)		
	image.anchorX = 0
	image.anchorY = 0
	sceneGroup:insert(image)
	--Display pokemon name and pokedex info
	local name = display.newText( "Charmander", display.contentWidth*(5/8) , 60, native.systemFont, 20 )
	name:setFillColor( 0, 0, 0 )
	sceneGroup:insert(name)
	local poketype = display.newText( "The Lizard Pokemon", display.contentWidth*(5/8), 80, native.systemFont, 15)
	...
	local redblueEntry = display.newText("Obviously prefers hot places. When it rains, steam is said to spout from the tip of its tail.", display.contentWidth/2, 170, 310,0,right,native.systemFont, 15)
	...
	local yellowEntry = display.newText("The flame at the tip of its tail makes a sound as it burns. You can only hear it in quiet places.", display.contentWidth/2, 350,310,0,right, native.systemFont, 15)
