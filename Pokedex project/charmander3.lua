...
-- Charmander page 3 for the pokedex
...
local function handleButtonEvent( event )
    if ( "ended" == event.phase ) then
		composer.gotoScene("charmander4")
...
local function handlepreviousPageEvent (event)
	if  ( "ended" == event.phase) then 
		composer.gotoScene("charmander2")
	end
end
...
local previousPage = widget.newButton(
	...
	local GoldEntry = display.newText("The flame on its tail shows the strength of its life force. If it is weak, the flame also burns weakly.", display.contentWidth/2, 175,310,0,right, native.systemFont, 15)
	...
	local SilverEntry = display.newText("The flame on its tail indicates Charmander's life force. If it is healthy, the flame burns brightly.", display.contentWidth/2, 330,310,0,right, native.systemFont, 15)
	SilverEntry:setFillColor (0,0,0)
	sceneGroup: insert(SilverEntry)
	
		--Crystal
	local Crystal = display.newText("Crystal Pokedex entry:", display.contentWidth/2, 370, native.systemFont, 17)
	Crystal:setFillColor (0,0,0)
	sceneGroup: insert(Crystal)
	local CrystalEntry = display.newText("If it's healthy, the flame on the tip of its tail will burn vigorously, even if it gets a bit wet.", display.contentWidth/2, 400,310,0,right, native.systemFont, 15)
