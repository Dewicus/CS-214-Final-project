...
-- Metapod page 1 for the pokedex
...
local function playcry (event)
	if  ( "ended" == event.phase) then 
		local cry = audio.loadSound("11.mp3")
...
local previousPage = widget.newButton(
	...
	local image = display.newImageRect( "011.png", 120,120)		
	image.anchorX = 0
	image.anchorY = 0
	sceneGroup:insert(image)
	--Display pokemon name and pokedex info
	local name = display.newText( "Metapod", display.contentWidth*(5/8) , 60, native.systemFont, 20 )
	...
	local redblueEntry = display.newText("This Pokémon is vulnerable to attack while its shell is soft, exposing its weak and tender body.", display.contentWidth/2, 170, 310,0,right,native.systemFont, 15)
	...
	local yellowEntry = display.newText("Hardens its shell to protect itself. However, a large impact may cause it to pop out of its shell.", display.contentWidth/2, 350,310,0,right, native.systemFont, 15)
