...
-- Charizard page 1 for the pokedex
...
local function playcry (event)
	if  ( "ended" == event.phase) then 
		local cry = audio.loadSound("6.mp3")
...
local previousPage = widget.newButton(
	...
	local image = display.newImageRect( "006.png", 120,120)		
	image.anchorX = 0
	image.anchorY = 0
	sceneGroup:insert(image)
	--Display pokemon name and pokedex info
	local name = display.newText( "Charizard", display.contentWidth*(5/8) , 60, native.systemFont, 20 )
	name:setFillColor( 0, 0, 0 )
	sceneGroup:insert(name)
	local poketype = display.newText( "The Flame Pokemon", display.contentWidth*(5/8), 80, native.systemFont, 15)
	...
	local redblueEntry = display.newText("Spits fire that is hot enough to melt boulders. Known to cause forest fires unintentionally.", display.contentWidth/2, 170, 310,0,right,native.systemFont, 15)
	...
	local yellowEntry = display.newText("When expelling a blast of super hot fire, the red flame at the tip of its tail burns more intensely.", display.contentWidth/2, 350,310,0,right, native.systemFont, 15)
