...
-- Charmeleon page 5 for the pokedex
...
local function handleButtonEvent( event )
    if ( "ended" == event.phase ) then
		composer.gotoScene("charmeleon6")
...
local previousPage = widget.newButton(
	...
	local blEntry = display.newText("In the rocky mountains where Charmeleon live, their fiery tails shine at night like stars.", display.contentWidth/2, 175,310,0,right, native.systemFont, 15)
	...
	sceneGroup: insert(x)
	local xEntry = display.newText("It lashes about with its tail to knock down its foe. It then tears up the fallen opponent with sharp claws.", display.contentWidth/2, 330,310,0,right, native.systemFont, 15)
	...
	sceneGroup: insert(y)
	local yEntry = display.newText("When it swings its burning tail, it elevates the air temperature to unbearably high levels.", display.contentWidth/2, 405,310,0,right, native.systemFont, 15)
