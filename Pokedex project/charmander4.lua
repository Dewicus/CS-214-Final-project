...
-- Charmander page 4 for the pokedex
...
local function handleButtonEvent( event )
    if ( "ended" == event.phase ) then
		composer.gotoScene("charmander5")
...
local previousPage = widget.newButton(
	...
	local rseEntry = display.newText("The flame that burns at the tip of its tail is an indication of its emotions. The flame wavers when Charmander is enjoying itself. If the Pokémon becomes enraged, the \n\n\n\n\n\nflame burns fiercely.", display.contentWidth/2, 215,312,0,right, native.systemFont, 15)
	rseEntry:setFillColor (0,0,0)
	sceneGroup: insert(rseEntry)
	
	--dimond pearl and platinum
	local dpp = display.newText("   Diamond, Pearl and Platinum Pokedex \n                          entries:", display.contentWidth/2, 330,310,0,center, native.systemFont, 17)
	dpp:setFillColor (0,0,0)
	sceneGroup: insert(dpp)
	local dppEntry = display.newText("The fire on the tip of its tail is a measure of its life. If healthy, its tail burns intensely.", display.contentWidth/2, 370,310,0,right, native.systemFont, 15)
