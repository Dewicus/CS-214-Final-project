...
local function playcry (event)
	if  ( "ended" == event.phase) then 
		local cry = audio.loadSound("150.mp3")
...
local previousPage = widget.newButton(
	...
	local image = display.newImageRect( "150.png", 120,120)		
	image.anchorX = 0
	image.anchorY = 0
	sceneGroup:insert(image)
	--Display pokemon name and pokedex info
	local name = display.newText( "Mewtwo", display.contentWidth*(5/8) , 60, native.systemFont, 20 )
	name:setFillColor( 0, 0, 0 )
	sceneGroup:insert(name)
	local poketype = display.newText( "Genetic Pokemon", display.contentWidth*(5/8), 80, native.systemFont, 15)
	...
	local redblueEntry = display.newText("It was created by a scientist after years of horrific gene splicing and DNA engineering experiments.", display.contentWidth/2, 170, 310,0,right,native.systemFont, 15)
	...
	local yellowEntry = display.newText("Its DNA is almost the same as Mew's. However, its size and disposition are vastly different.", display.contentWidth/2, 350,310,0,right, native.systemFont, 15)
