...
-- Venusaur page 1 for the pokedex
...
local function playcry (event)
	if  ( "ended" == event.phase) then 
		local cry = audio.loadSound("3.mp3")
...
local previousPage = widget.newButton(
	...
	local image = display.newImageRect( "003.png", 120,120)		
	image.anchorX = 0
	image.anchorY = 0
	sceneGroup:insert(image)
	--Display pokemon name and pokedex info
	local name = display.newText( "Venusaur", display.contentWidth*(5/8) , 60, native.systemFont, 20 )
	...
	local redblueEntry = display.newText("The plant blooms when it is absorbing solar energy. It stays on the move to seek sunlight.", display.contentWidth/2, 170, 310,0,right,native.systemFont, 15)
	...
	local yellowEntry = display.newText("The flower on its back catches the sun's rays. The sunlight is then absorbed and used for energy.", display.contentWidth/2, 350,310,0,right, native.systemFont, 15)
